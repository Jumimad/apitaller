...
final class PdoDatabase implements Database {
	...
	public function insert($tableName, array $insertMappings, $onupdate = false, $campo=false, $value=false) {
		...
		if($onupdate){
			$statement = 'INSERT INTO '.$tableName.' ('.$columnList.') VALUES ('.$placeholderList.') ON DUPLICATE KEY UPDATE '.$campo.'="'.$value.'" ;';
		...
		//var_dump($statement);
		//var_dump(array_values($insertMappings));
