...
final class Auth extends UserManager {
	...
	const SESSION_FIELD_DATA = 'auth_data';
	const SESSION_FIELD_DATAF = 'auth_dataf';
	const SESSION_FIELD_NUMDOC = 'auth_numDoc';
	const SESSION_FIELD_CASH = 'auth_cash';
	const SESSION_FIELD_GANANCIA = 'auth_ganancia';
	const SESSION_FIELD_NOMBRE = 'auth_nombre';
	const SESSION_FIELD_LASTLOGIN = 'auth_lastlogin';
	const SESSION_FIELD_TAQUILLA = 'auth_taquilla';
	const SESSION_FIELD_IP = 'auth_ip';
	const SESSION_FIELD_PREGUNTA_SEGURIDAD = 'auth_pregunta_seguridad';
	const SESSION_FIELD_PERFIL = 'auth_perfil';
	const SESSION_FIELD_C_FECHA = 'auth_c_fecha';
	const SESSION_FIELD_TIPO_REGISTRO = 'auth_tipo_registro';
	const SESSION_FIELD_REGISTER_AT = 'auth_tipo_register_at';
	const SESSION_FIELD_STATE = 'auth_state';
	const SESSION_FIELD_TIMEONLINE = 'auth_timeOnline';
	
	const SESSION_FIELD_FIRSTNAME = 'auth_firstname';
	const SESSION_FIELD_SECONDNAME = 'auth_secondname';
	const SESSION_FIELD_LASTNAME = 'auth_lastname';

	
	const SESSION_FIELD_STATUS = 'auth_status';
	const SESSION_FIELD_ROLES = 'auth_roles';
	const SESSION_FIELD_ROLE = 'auth_role';
	...
	public function __construct($databaseConnection, $useHttps = false, $allowCookiesScriptAccess = false, $ipAddress = null, $dbTablePrefix = null) {
		...
		$this->ipAddress = empty($ipAddress) ? (isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR']:'127.0.0.1') : $ipAddress;
				...
				if (isset($parts[0]) && isset($parts[1])) {
					try {
						$rememberData = $this->db->selectRow(
							'SELECT a.site_id, a.ciudad_res, b.role, a.cod_postal,a.depart_res,a.dir,a.movil,a.lastname,a.firstname,a.pais,a.secondname,a.sexo,a.tipo_doc,a.user, a.token, a.expires, b.email, b.username, b.status, b.roles_mask, b.numDoc, b.cash, b.ganancia, b.nombre, b.last_login, b.taquilla, b.ip, b.pregunta_seguridad, b.perfil, b.c_fecha, b.tipo_registro, b.register_at, b.state, b.timeOnline FROM ' . $this->dbTablePrefix . 'users_remembered AS a JOIN ' . $this->dbTablePrefix . 'users AS b ON a.user = b.id WHERE a.selector = ?',
							[ $parts[0] ]
						);
					}
					catch (Error $e) {
					...
					if (!empty($rememberData)) {
						if ($rememberData['expires'] >= time()) {
							if (password_verify($parts[1], $rememberData['token'])) {
								$this->onLoginSuccessful($rememberData['user'], $rememberData['email'], $rememberData['username'], $rememberData['status'], $rememberData['roles_mask'], true, $rememberData);
							}
						}
					}
				}
	 ...
	 */
	public function register($data = null, callable $callback = null) {
		global $config;
		...
		$newUserId = $this->createUserInternal(false, $data, $callback);
	 ...
	 * @see confirmEmailAndSignIn
	 */
	public function registerWithUniqueUsername($data = null, callable $callback = null) {
		$this->throttle([ 'enumerateUsers', $this->getIpAddress() ], 1, (60 * 60), 75);
		$this->throttle([ 'createNewAccount', $this->getIpAddress() ], 1, (60 * 60 * 12), 5, true);
		$data['ip'] = $this->getIpAddress();
		$newUserId = $this->createUserInternal(true, $data, $callback);
	...
	public function loginID($userId) {
		$userData = $this->db->selectRow(
			'SELECT * FROM ' . $this->dbTablePrefix . 'users WHERE id = ? and status = 1',
			[ $userId ]
		);
		$this->onLoginSuccessful($userData['id'], $userData['email'], $userData['username'], $userData['status'], $userData['roles_mask'], false, $userData);
	}
	...
	public function setGanancia2($importe, $userid=false, $debug=false){

        if($userid){
            $userid = (int)$userid;
        }else{
            $userid = (int)$_SESSION["auth_dataf"]['id'];
        }
        if($debug){
        	
        	//$SQL = "update ".$this->dbTablePrefix."users set ganancia = '".$importe."' where id = ".$userid." ";
        	//var_dump($SQL);
        }
        //$SQL = "update users set cash = '".$importe."' where id = ".$userid." ";
        $this->db->update(
	...
	public function setCash2($importe, $userid=false){
    ...
    public function setCash4($importe, $taquillaid=false){

        $taquillaid = (int)$taquillaid;
        
        //$SQL = "update users set cash = '".$importe."' where id = ".$userid." ";
        $this->db->update(
			$this->dbTablePrefix . 'Taquilla',
			[ 'saldo_taquilla' => $importe ],
			[ 'id' => $taquillaid ]
		);
    
    }
    
     public function setCash3($importe, $intermediarioId=false){

        $intermediarioId = (int)$intermediarioId;
        
        //$SQL = "update users set cash = '".$importe."' where id = ".$userid." ";
        $this->db->update(
			$this->dbTablePrefix . 'Intermediario',
			[ 'saldo_intermediario' => $importe ],
			[ 'id' => $intermediarioId ]
		);
		
    }
    
    public function insertSanitized($table, $data){
    	$this->db->insert(
				$this->dbTablePrefix . $table,
				[
					$Log
				]
			);
	...
	public function getUserData($userId=false){
		if($userId){
            ...
            $userId = (int)isset($_SESSION["auth_dataf"]['id']) ? $_SESSION["auth_dataf"]['id']:0;
        }
        
		try {
			$userData = $this->db->selectRow(
				'SELECT * FROM ' . $this->dbTablePrefix . 'users WHERE id = ?',
				[ $userId ]
			);
			$taquillaData = $this->db->selectRow(
				'SELECT * FROM ' . $this->dbTablePrefix . 'Taquilla WHERE UserID = ?',
				[ $userId ]
			);
			
			$userData['estacion'] = isset($_SESSION['esEstacion'])?$_SESSION['esEstacion']:false;
			$userData['Taquilla'] = $taquillaData;
			
			$Intermediario = $this->db->selectRow(
				'SELECT * FROM ' . $this->dbTablePrefix . 'Intermediario WHERE UserID = ?',
				[ $userId ]
			);
			$userData['Intermediario'] = $Intermediario;
		}
		catch (Error $e) {
			throw new DatabaseError();
		}
	...
	private function onLoginSuccessful($userId, $email, $username, $status, $roles, $remembered, $dataExtra=array()) {
		$tm = ((microtime(true)+35) * 1000);
		try {
			$this->db->update(
				$this->dbTablePrefix . 'users',
				[ 'last_login' => time(), 'timeOnline'=>$tm ],
				[ 'id' => $userId ]
			);
		}
		catch (Error $e) {
		...
		$dataExtra['Taquilla'] = $taquillaData;
		$dataExtra['estacion'] = $_SESSION['esEstacion'];
		// save the user data in the session
		$this->setDataF($dataExtra);
		unset($dataExtra["password"]);
		...
		$this->setNumDoc($dataExtra['numDoc']);
		$this->setCash($dataExtra['cash']);
		$this->setGanancia($dataExtra['ganancia']);
		$this->setNombre($dataExtra['nombre']);
		$this->setFirstName($dataExtra['firstname']);
		$this->setSecondName($dataExtra['secondname']);
		$this->setLastName($dataExtra['lastname']);
		$this->setLastLogin($dataExtra['last_login']);
		$this->setTaquilla($dataExtra['taquilla']);
		$this->setIP($dataExtra['ip']);
		$this->setRole($dataExtra['role']);
		$this->setPreguntaSeguridad($dataExtra['pregunta_seguridad']);
		$this->setPerfil($dataExtra['perfil']);
		$this->setCFecha($dataExtra['c_fecha']);
		$this->setTipoRegistro($dataExtra['tipo_registro']);
		$this->setRegisterAt($dataExtra['register_at']);
		$this->setState($dataExtra['state']);
		$this->setTimeOnline($tm);
		...
		if (!$this->isLoggedIn()) {
			if ($verifiedEmail !== null) {
				...
				$userData = $this->getUserDataByEmailAddress(
					$verifiedEmail,
					[ 'id', 'email', 'username','ciudad_res', 'cod_postal','depart_res','dir','movil','lastname','firstname','pais','secondname','sexo','tipo_doc', 'status', 'role', 'roles_mask', 'numDoc', 'cash', 'ganancia', 'nombre', 'last_login', 'taquilla', 'ip', 'pregunta_seguridad', 'perfil', 'c_fecha', 'tipo_registro', 'register_at', 'state', 'timeOnline']
				);

				$this->onLoginSuccessful($userData['id'], $userData['email'], $userData['username'], $userData['status'], $userData['roles_mask'], true, $userData);
	...
	public function checkPassword($password, $cc){
		    $userData = $this->db->selectRow(
				'SELECT * FROM ' . $this->dbTablePrefix . 'users WHERE numDoc = ?',
				[ $cc ]
			);
		$password = self::validatePassword($password);
	...
	private function authenticateUserInternal($password, $email = null, $username = null, $rememberDuration = null, callable $onBeforeSuccess = null) {
		...
		$_SESSION['esEstacion'] = false;
		$columnsToFetch = [ 'site_id', 'id', 'email', 'username','password','verified', 'ciudad_res', 'cod_postal','depart_res','dir','movil','lastname','firstname','pais','secondname','sexo','tipo_doc', 'role', 'status', 'roles_mask', 'numDoc', 'cash', 'ganancia', 'nombre', 'last_login', 'taquilla', 'ip', 'pregunta_seguridad', 'perfil', 'c_fecha', 'tipo_registro', 'register_at', 'state', 'timeOnline'];

		if ($email !== null) {
			
			//var_dump(empty($userData));
		
				if(is_int((int)$email) && ((int)$email)>0 ){
					$email_ = trim((int)$email);
					//var_dump($email_);
					//var_dump($columnsToFetch);
					...
					//var_dump($userData);
				}else if(strpos('_'.$email,'admin_')>0){
					$userData = $this->getUserDataByUserTaquilla(
						$email,
						$columnsToFetch
					);
		...
		if(isset($userData['status']) && $userData['status'] != 1){
			die('{"state":8,"msg":"Su cuenta está suspendida o se encuentra en revision. Contacte a un administrador."}');
		}
		...
		if (password_verify($password, $userData['password'])) {
			...
			if ((int) $userData['verified'] === 1) {
				if (!isset($onBeforeSuccess) || (\is_callable($onBeforeSuccess) && $onBeforeSuccess($userData['id']) === true)) {
					...
					if(strpos('_'.$email,'admin_')>0){
						$_SESSION['esEstacion'] = true;	
					}
					if($userData['id'] == '150'){
			            //var_dump($_SESSION);
			        }
	...
	private function getUserDataByUserTaquilla($email, array $requestedColumns) {
		global $config;
		try {
			...
			$TaquillaData = $this->db->selectRow(
				'SELECT id,UserID FROM ' . $this->dbTablePrefix . 'Taquilla WHERE (site_id = "'.(int)$config['site_id'].'") and cuenta_recargas = ?',
				[ $email ]
			);
			
			//var_dump($TaquillaData);
			
			$userData = $this->db->selectRow(
				'SELECT ' . $projection . ' FROM ' . $this->dbTablePrefix . 'users WHERE site_id = "'.(int)$config['site_id'].'" and id = ?',
				[ $TaquillaData['UserID'] ]
			);
		}
		catch (Error $e) {
	...
	private function getUserDataByEmailAddress($email, array $requestedColumns) {
		...
		try {
			$projection = implode(', ', $requestedColumns);
			$userData = $this->db->selectRow(
				'SELECT ' . $projection . ' FROM ' . $this->dbTablePrefix . 'users WHERE site_id = "'.(int)$config['site_id'].'" and email = ?',
				[ $email ]
			);
		}
		catch (Error $e) {
	...
	public function resetPassword($selector, $token, $newPassword) {
		...
		try {
			$resetData = $this->db->selectRow(
				'SELECT a.id, a.user,b.role,  a.token, a.expires, b.resettable FROM ' . $this->dbTablePrefix . 'users_resets AS a JOIN ' . $this->dbTablePrefix . 'users AS b ON b.id = a.user WHERE a.selector = ?',
				[ $selector ]
			);
		}
		catch (Error $e) {
	...
	private function setNumDoc($numdoc) {
		$_SESSION[self::SESSION_FIELD_NUMDOC] = $numdoc;
	}
	private function setCash($cash) {
		$_SESSION[self::SESSION_FIELD_CASH] = $cash;
	}
	private function setGanancia($ganancia) {
		$_SESSION[self::SESSION_FIELD_GANANCIA] = $ganancia;
	}
	private function setNombre($nombre) {
		$_SESSION[self::SESSION_FIELD_NOMBRE] = $nombre;
	}
	private function setLastLogin($lastlogin) {
		$_SESSION[self::SESSION_FIELD_LASTLOGIN] = $lastlogin;
	}
	private function setTaquilla($taquilla) {
		$_SESSION[self::SESSION_FIELD_TAQUILLA] = $taquilla;
	}
	private function setIP($ip) {
		$_SESSION[self::SESSION_FIELD_IP] = $ip;
	}

	private function setPreguntaSeguridad($pregunta) {
		$_SESSION[self::SESSION_FIELD_PREGUNTA_SEGURIDAD] = $pregunta;
	}
	private function setPerfil($perfil) {
		$_SESSION[self::SESSION_FIELD_PERFIL] = $perfil;
	}
	private function setCFecha($fecha) {
		$_SESSION[self::SESSION_FIELD_C_FECHA] = $fecha;
	}
	private function setTipoRegistro($tiporegistro) {
		$_SESSION[self::SESSION_FIELD_TIPO_REGISTRO] = $tiporegistro;
	}
	private function setRegisterAt($registerat) {
		$_SESSION[self::SESSION_FIELD_REGISTER_AT] = $registerat;
	}
	private function setState($state) {
		$_SESSION[self::SESSION_FIELD_STATE] = $state;
	}
	private function setTimeOnline($timeonline) {
		$_SESSION[self::SESSION_FIELD_TIMEONLINE] = $timeonline;
	}
	
	private function setFirstName($firstname) {
		$_SESSION[self::SESSION_FIELD_FIRSTNAME] = $firstname;
	}
	private function setSecondName($secondname) {
		$_SESSION[self::SESSION_FIELD_SECONDNAME] = $secondname;
	}
	private function setLastName($lastname) {
		$_SESSION[self::SESSION_FIELD_LASTNAME] = $lastname;
	}
	
	private function setRole($role) {
		$_SESSION[self::SESSION_FIELD_ROLE] = $role;
	}
	
	
		
	public function getRole() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_ROLE])) {
			return $_SESSION[self::SESSION_FIELD_ROLE];
		}
		else {
			return null;
		}
	...
	public function getNumDoc() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_NUMDOC])) {
			return $_SESSION[self::SESSION_FIELD_NUMDOC];
		}
		else {
			return null;
		}
	...
	public function getCash() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_CASH])) {
			return $_SESSION[self::SESSION_FIELD_CASH];
		}
		else {
			return null;
		}
	...
	public function getGanancia() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_GANANCIA])) {
			return $_SESSION[self::SESSION_FIELD_GANANCIA];
		}
		else {
			return null;
		}
	...
	public function getNombre() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_NOMBRE])) {
			return $_SESSION[self::SESSION_FIELD_NOMBRE];
		}
		else {
			return null;
		}
	...
	public function getFirstName() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_FIRSTNAME])) {
			return $_SESSION[self::SESSION_FIELD_FIRSTNAME];
		}
		else {
			return null;
		}
	...
	public function getSecondName() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_SECONDNAME])) {
			return $_SESSION[self::SESSION_FIELD_SECONDNAME];
		}
		else {
			return null;
		}
	...
	public function getLastName() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_LASTNAME])) {
			return $_SESSION[self::SESSION_FIELD_LASTNAME];
		}
		else {
			return null;
		}
	...
	public function getLastLogin() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_LASTLOGIN])) {
			return $_SESSION[self::SESSION_FIELD_LASTLOGIN];
		}
		else {
			return null;
		}
	...
	public function getTaquilla() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_TAQUILLA])) {
			return $_SESSION[self::SESSION_FIELD_TAQUILLA];
		}
		else {
			return null;
		}
	...
	public function getIP() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_IP])) {
			return $_SESSION[self::SESSION_FIELD_IP];
		}
		else {
			return null;
		}
	...
	public function getPreguntaSeguridad() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_PREGUNTA_SEGURIDAD])) {
			return $_SESSION[self::SESSION_FIELD_PREGUNTA_SEGURIDAD];
		}
		else {
			return null;
		}
	...
	public function getPerfil() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_PERFIL])) {
			return $_SESSION[self::SESSION_FIELD_PERFIL];
		}
		else {
			return null;
		}
	...
	public function getCFecha() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_C_FECHA])) {
			return $_SESSION[self::SESSION_FIELD_C_FECHA];
		}
		else {
			return null;
		}
	...
	public function getTipoRegistro() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_TIPO_REGISTRO])) {
			return $_SESSION[self::SESSION_FIELD_TIPO_REGISTRO];
		}
		else {
			return null;
		}
	...
	public function getRegisterAt() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_REGISTER_AT])) {
			return $_SESSION[self::SESSION_FIELD_REGISTER_AT];
		}
		else {
			return null;
		}
	...
	public function getState() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_STATE])) {
			return $_SESSION[self::SESSION_FIELD_STATE];
		}
		else {
			return null;
		}
	...
	public function getTimeOnline() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_TIMEONLINE])) {
			return $_SESSION[self::SESSION_FIELD_TIMEONLINE];
		}
		else {
			return null;
		}
	...
	public function setData($data) {
		$_SESSION[self::SESSION_FIELD_DATA] = $data;
	}
	public function setDataF($data) {
		$_SESSION[self::SESSION_FIELD_DATAF] = $data;
	}
	...
	public function getData() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_DATA])) {
			return $_SESSION[self::SESSION_FIELD_DATA];
		}
		else {
			return null;
		}
	...
	public function getDataF() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_FIELD_DATAF])) {
			return $_SESSION[self::SESSION_FIELD_DATAF];
		}
		else {
			return null;
		}
